...
import { DFCard, DFBadge, DFAvatar } from "./ui/index";
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {QUESTIONNAIRE_OPTIONS.recordingPreference.map(pref => {
                ...
                return (
                    <DFCard key={pref.id} noPadding className="flex flex-col h-full hover:border-df-primary/20 transition-all duration-300">
                        <div className="p-4 border-b border-df-border-subtle flex items-center justify-between bg-df-primary/5">
                            <h3 className="df-label !text-[10px] text-df-primary tracking-widest uppercase">
                                {pref.label}
                            </h3>
                        ...
                        <div className="flex-1 p-3 max-h-[340px] overflow-y-auto custom-scrollbar">
                            {list.length === 0 ? (
                                <div className="py-12 text-center opacity-40 italic">
                                    <p className="df-caption">Sin alumnos segmentados</p>
                                </div>
                            ) : (
                                <div className="space-y-1">
                                    {list.map(user => (
                                        ...
                                        >
                                            ...
                                            <span className="df-body-sm text-df-text-soft group-hover:text-df-primary transition-colors font-medium">
                                                {user.username}
                                            </span>
                                        </button>
                                    ))}
