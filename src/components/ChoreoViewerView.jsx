...
const ChoreoViewerView = ({ onTabChange }) => {
  ...
  const renderExplorer = () => (
    <div className="p-1.5 space-y-2">
      <div className="space-y-0.5 px-1">
        <h2 className="text-xl font-black uppercase tracking-tighter text-white">Explorar</h2>
      ...
      <div className="grid gap-1.5">
        {filteredChoreos.map(choreo => (
          <div
            ...
            className="bg-surface/40 backdrop-blur-xl border border-outline/60 p-1.5 rounded-xl flex items-center gap-2 group active:scale-95 transition-all"
          >
            <div
              className="w-10 h-10 rounded-lg flex flex-col items-center justify-center text-white shadow-xl shrink-0"
              style={{ backgroundColor: choreo.color }}
            >
              <span className="text-xs font-black uppercase">{choreo.difficulty?.substring(0, 1)}</span>
              <Zap size={12} className="mt-0 opacity-60" />
            </div>

            <div className="flex-1 min-w-0">
              <h4 className="font-black text-sm text-white truncate uppercase tracking-tight">{choreo.title}</h4>
              <p className="text-xs font-bold text-zinc-500 uppercase tracking-tighter truncate">
  ...
  const renderPlayer = () => {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-black/10 backdrop-blur-sm pb-32">
        {/* Header */}
        <div className="p-1.5 flex items-center justify-between">
          ...
          <div className="text-center flex-1 truncate px-1">
            ...
            <p className="text-xs font-black uppercase tracking-widest mt-0">
        ...
        <div className="px-1.5 py-1 space-y-1">
          ...
          <div className="flex gap-1">
            {[0, 4].map(startOffset => (
               <div key={startOffset} className="grid grid-cols-4 gap-1 flex-1">
                 {Array.from({ length: 4 }).map((_, i) => {
                   ...
                   return (
                     <div
                       ...
                       className={`
                         h-12 rounded-lg flex items-center justify-center transition-all duration-150 relative border
                         ${isActive ? 'border-primary shadow-[0_0_40px_rgba(225,29,72,0.4)] z-10' : 'border-white/5'}
                       `}
                     >
                       {block && (
                         <span className="text-xs font-black text-white uppercase truncate px-1 text-center drop-shadow-md leading-tight">
                           {block.name}
                         </span>
                       )}
                       {isActive && (
               ...
               <div key={startOffset} className="grid grid-cols-4 gap-1 flex-1">
                 {Array.from({ length: 4 }).map((_, i) => {
                   ...
                   return (
                     <div
                       key={slotIdx}
                       className={`h-8 flex items-center justify-center rounded-lg border transition-all duration-150
                         ${isActive ? 'bg-primary/40 border-primary shadow-[0_0_20px_rgba(225,29,72,0.3)]' : 'bg-white/5 border-white/5'}
                       `}
                     >
                       <span className={`text-xs font-black ${isActive ? 'text-white' : 'text-white/20'}`}>{(slotIdx % 8) + 1}</span>
                     </div>
                   );
                 })}
               </div>
        ...
        {/* Detail Card */}
        <div className="flex-1 px-1.5 mb-1.5">
          <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-2xl p-3 space-y-2 shadow-2xl min-h-[160px] flex flex-col">
            {activeBlock ? (
              <>
                <div className="flex items-center gap-2">
                  <div
                    className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-black text-base shadow-2xl shrink-0"
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] mt-0.5">
                ...
                <div className="space-y-2 flex-1">
                  <div className="space-y-0.5">
                     ...
                     <p className="text-sm text-white font-medium italic leading-tight opacity-95">
        ...
        {/* Playback Controls */}
        <div className="fixed bottom-[72px] left-1.5 right-1.5 z-40">
           <div className="bg-black/95 backdrop-blur-3xl border border-white/10 rounded-2xl p-2.5 flex items-center justify-between shadow-2xl">
              <div className="flex items-center gap-1.5">
                 <button
                   onClick={isPlaying ? pausePlayback : () => startPlayback(bpm)}
                   className="w-10 h-10 rounded-full bg-rose-500 flex items-center justify-center text-white shadow-lg shadow-rose-500/30 active:scale-90 transition-all"
                 >
                   {isPlaying ? <Pause size={20} fill="white" /> : <Play size={20} fill="white" className="ml-0.5" />}
                 </button>
                 <button
                   onClick={stopPlayback}
                   className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-black active:scale-90 transition-all shadow-lg"
                 >
              ...
              <button
                ...
                className="bg-zinc-800/80 text-white/90 px-2 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] border border-white/10 active:scale-95 transition-all shadow-xl whitespace-nowrap"
              >
                MODO
              </button>
           </div>
